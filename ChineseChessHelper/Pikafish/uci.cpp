...
UCIEngine::UCIEngine(std::string filePath) :
    engine(filePath) {
...
void UCIEngine::execute(std::string cmd) {
    sync_cout << "execute: " << cmd << sync_endl;
...
bool UCIEngine::check_move(const std::string &postion, const std::string &move) {
    std::istringstream is(postion);
